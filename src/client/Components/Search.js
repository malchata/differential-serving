...
export default class Search extends Component {
  constructor (props) {
    ...
    if ("sortOrder" in this.props && /^(asc|desc)$/i.test(this.props.sortOrder)) {
      sortOrder = this.props.sortOrder;
    }

    if ("sortBy" in this.props && /^(model|type|manufacturer)$/i.test(this.props.sortBy)) {
  ...
  getPedalData () {
    ...
    if (searchQuery.length > 0) {
      fetch(`/api/search/${encodeURIComponent(searchQuery)}`).then(response => response.json()).then(data => {
        let pedals = [];
        let sortKey = "model";
        ...
        const json = simpleSort(data, sortKey, sortOrder);
